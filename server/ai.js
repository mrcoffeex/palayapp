...
export function answerAssistant({ role, user, message, db }) {
  ...
  if (!q) {
    return {
      reply: "Ask me anything about products, orders, pricing, or how PalayUP works.",
...
function greeting(role, user) {
  ...
  if (role === "admin") {
    return `Hello ${first}. I’m the PalayUP admin assistant. I can summarize users, products, orders, chats, notifications, and guide prices.`;
  }
  if (role === "farmer") {
...
function buyerAnswer(text, user, db, activeProducts) {
  ...
  if (includesAny(text, ["how do i order", "place order", "buy"])) {
    return {
      reply:
        "Open a product, check the guide price, choose quantity, and place the order. PalayUP then shows the farmer’s phone and farm location. Arrange pickup or delivery directly — the app only tracks status, it does not take payment.",
  ...
  if (includesAny(text, ["contact", "phone", "location", "where", "farm"])) {
    return {
      reply:
        "On every product and order you’ll see the farmer’s farm name, phone number, and full address. Call or message them in Chat to arrange collection. No payment happens inside PalayUP.",
